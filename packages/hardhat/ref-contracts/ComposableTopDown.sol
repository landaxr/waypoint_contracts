...
ERC998ERC20TopDown, ERC998ERC20TopDownEnumerable {
    ...
    function rootOwnerOfChild(address _childContract, uint256 _childTokenId) public view returns (bytes32 rootOwner) {
        ...
        cdata = abi.encodeWithSelector(0xed81cdda, address(this), _childTokenId);
        assembly {
            callSuccess := staticcall(gas(), rootOwnerAddress, add(cdata, 0x20), mload(cdata), cdata, 0x20)
            if callSuccess {
                rootOwner := mload(cdata)
            }
        }
    ...
    function _transferFrom(address _from, address _to, uint256 _tokenId) private {
        ...
        if(msg.sender != _from) {
            ...
            bool callSuccess;
            // 0xed81cdda == rootOwnerOfChild(address,uint256)
            bytes memory cdata = abi.encodeWithSelector(0xed81cdda, address(this), _tokenId);
            assembly {
                callSuccess := staticcall(gas(), _from, add(cdata, 0x20), mload(cdata), cdata, 0x20)
                if callSuccess {
                    rootOwner := mload(cdata)
                }
            }
    ...
    function transferChild(uint256 _fromTokenId, address _to, address _childContract, uint256 _childTokenId) external {
        ...
        bytes memory cdata = abi.encodeWithSelector(0x095ea7b3, this, _childTokenId);
        assembly {
            let success := call(gas(), _childContract, 0, add(cdata, 0x20), mload(cdata), cdata, 0)
        }
    ...
    function getERC20(address _from, uint256 _tokenId, address _erc20Contract, uint256 _value) public {
        ...
        if (!allowed) {
            ...
            bytes memory cdata = abi.encodeWithSelector(0xdd62ed3e, _from, msg.sender);
            bool callSuccess;
            assembly {
                callSuccess := staticcall(gas(), _erc20Contract, add(cdata, 0x20), mload(cdata), cdata, 0x20)
                if callSuccess {
                    remaining := mload(cdata)
                }
            }
