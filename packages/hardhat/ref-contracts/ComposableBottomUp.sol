...
contract ComposableBottomUp is ERC721, ERC998ERC721BottomUp, ERC998ERC721BottomUpEnumerable {
    ...
    */
    ...
    function rootOwnerOf(uint256 _tokenId) public view returns (bytes32 rootOwner) {
        ...
        if (isParent == false) {
            ...
            cdata = abi.encodeWithSelector(0xed81cdda, address(this), _tokenId);
            assembly {
            ...
            cdata = abi.encodeWithSelector(0x43a61a8e, parentTokenId);
            assembly {
                callSuccess := staticcall(gas(), rootOwnerAddress, add(cdata, 0x20), mload(cdata), cdata, 0x20)
                if callSuccess {
                    rootOwner := mload(cdata)
            ...
            if (callSuccess == true && rootOwner >> 224 == ERC998_MAGIC_VALUE) {
                ...
                cdata = abi.encodeWithSelector(0x6352211e, parentTokenId);
                assembly {
                    callSuccess := staticcall(gas(), rootOwnerAddress, add(cdata, 0x20), mload(cdata), cdata, 0x20)
                    if callSuccess {
                        rootOwnerAddress := mload(cdata)
                    }
                }
                require(callSuccess, "Call to ownerOf failed");

                // 0xed81cdda == rootOwnerOfChild(address,uint256)
                cdata = abi.encodeWithSelector(0xed81cdda, childContract, parentTokenId);
                assembly {
                    callSuccess := staticcall(gas(), rootOwnerAddress, add(cdata, 0x20), mload(cdata), cdata, 0x20)
                    if callSuccess {
                        rootOwner := mload(cdata)
  ...
  */
    ...
    function transferFromParent(address _fromContract, uint256 _fromTokenId, address _to, uint256 _tokenId, bytes _data) external {
        ...
        if (isContract(_to)) {
            bytes4 retval = IERC721TokenReceiver(_to).onERC721Received(msg.sender, _fromContract, _tokenId, _data);
